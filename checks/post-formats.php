			...
			foreach ( $checks as $check ) {
				...
				if ( preg_match( $check, $phpfile, $matches ) ) {
					if ( !strpos( $php, 'get_post_format' ) && !strpos( $php, 'has_post_format' ) && !strpos( $css, '.format' ) ) {
						...
						$this->error[] = sprintf('<span class="tc-lead tc-required">'.__('REQUIRED','theme-check-extended').'</span>: '.__('%1$s was found in the file %2$s. However get_post_format and/or has_post_format were not found, and no use of formats in the CSS was detected.', 'theme-check-extended'), '<strong>' . $error . '</strong>', '<strong>' . $filename . '</strong>');
						$ret = false;
					}
				}
			}
