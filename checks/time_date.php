		...
		foreach ( $php_files as $php_key => $phpfile ) {
			foreach ( $checks as $key => $check ) {
				...
				if ( preg_match( $key, $phpfile, $matches ) ) {
					...
					$this->error[] = sprintf( '<span class="tc-lead tc-info">' . __( 'INFO', 'theme-check-extended' ) . '</span>: ' . __( 'At least one hard coded date was found in the file %1$s. Consider %2$s instead.', 'theme-check-extended' ), '<strong>' . $filename . '</strong>', "<strong>get_option( 'date_format' )</strong>" );
				}
			}
		}
		return $ret;
