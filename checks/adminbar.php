		...
		function check( $php_files, $css_files, $other_files ) {
			...
			foreach ( $php_files as $file_path => $file_content ) {
				...
				if ( preg_match( $php_regex, $file_content, $matches ) ) {
					$this->error[] = sprintf( '<span class="tc-lead tc-required">' . __( 'REQUIRED', 'theme-check-extended').'</span>: ' . __( 'You are not allowed to hide the admin bar.', 'theme-check-extended' ),
			...
			foreach ( $css_files as $file_path => $file_content ) {
				...
				if ( preg_match( $css_regex, $file_content, $matches ) ) {
					$this->error[] = sprintf( '<span class="tc-lead tc-warning">' . __( 'WARNING', 'theme-check-extended').'</span>: ' . __( 'The theme is using `#wpadminbar`. Hiding the admin bar is not allowed.', 'theme-check-extended' ),
						'<strong>' . $filename . '</strong>');		
				}
			}
		return $ret;
