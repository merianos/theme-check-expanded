	...
	function check( $php_files, $css_files, $other_files ) {
		...
		if ( !empty( $data['AuthorURI'] ) && !empty( $data['URI'] ) ) {
			...
			if ( strtolower( preg_replace('/https?:\/\/|www./i', '', trim( $data['URI'] , '/' ) ) ) == strtolower( preg_replace('/https?:\/\/|www./i', '', trim( $data['AuthorURI'], '/' ) ) ) )  {
				$this->error[] = __('<span class="tc-lead tc-required">'.__('REQUIRED','theme-check-extended').'</span>: '.__('Your Theme URI and Author URI should not be the same.', 'theme-check-extended') );
				$ret = false;
			}
			...
			if ( stripos( $data['URI'], 'wordpress.org' ) && $data[ 'AuthorName' ] <> "the WordPress team" || stripos( $data['URI'], 'w.org' ) && $data[ 'AuthorName' ] <> "the WordPress team" ) {
				$this->error[] .= __('<span class="tc-lead tc-required">'.__('REQUIRED','theme-check-extended').'</span>: '.__('Using a WordPress.org Theme URI is reserved for official themes.', 'theme-check-extended') );
				$ret = false;
			}
		}
