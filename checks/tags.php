...
class TagCheck implements themecheck {
	...
	function check( $php_files, $css_files, $other_files ) {
		...
		if ( strpos( $php, 'the_tags' ) === false && strpos( $php, 'the_taxonomies' ) === false && strpos( $php, 'get_the_tag_list' ) === false && strpos( $php, 'get_the_term_list' ) === false ) {
			$this->error[] = "<span class='tc-lead tc-required'>" . __( 'REQUIRED', 'theme-check-extended' ) . '</span>: '. __( "This theme doesn't seem to display tags. Modify it to display tags in appropriate locations.", "theme-check" );
			$ret = false;
		}
