		...
		foreach ( $php_files as $php_key => $phpfile ) {
			foreach ( $checks as $key => $check ) {
				...
				if ( preg_match( '/[\s|\'|\"]' . $key . '(?:\'|"|;|\s)/', $phpfile, $matches ) ) {
					...
					$this->error[] = sprintf('<span class="tc-lead tc-required">'.__('REQUIRED','theme-check-extended').'</span>: '.__('%1$s was found in the file %2$s. Use %3$s instead.%4$s', 'theme-check-extended'), '<strong>' . $error . '</strong>', '<strong>' . $filename . '</strong>', '<strong>' . $check . '</strong>', $grep );
					$ret = false;
				}
			}
		}
