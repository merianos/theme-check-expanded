	...
	function check( $php_files, $css_files, $other_files ) {
		...
		foreach ( $php_files as $php_key => $phpfile ) {
			...
			if ( preg_match_all( "/(<a)(\\s+)(href" . $space_re . "=" . $space_re . "\"" . $space_re . "((http|https|ftp):\\/\\/)?)" . $url_re . "(\"" . $space_re . $title_re . $space_re . ">)" . $title_re . "(<\\/a>)/is", $phpfile, $out, PREG_SET_ORDER ) ) {
				...
				if ( $grep ) {
					$this->error[] = sprintf('<span class="tc-lead tc-info">'.__('INFO','theme-check-extended').'</span>: '.__('Possible hard-coded links were found in the file %1$s.%2$s', 'theme-check-extended'), '<strong>' . $filename . '</strong>', $grep);
				}
			}
		}
		return $ret;
